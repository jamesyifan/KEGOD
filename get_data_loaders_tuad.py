...
#         # print(g['x'].shape)
#         g['x'] = torch.cat([g['x'], RWSE, DGSE], dim=1)
#         # g['x'] = torch.cat([RWSE, DGSE], dim=1)
#         # print(g['x'].shape)
#         # import time
#         # time.sleep(100)
...
def get_ad_dataset_TU(args, split):
    DS = args.DS
    ...
    #     dataset = TUDataset(path, name=DS, pre_transform=pre_transform, use_node_attr=True)
    else:
        dataset = TUDataset(path, name=DS)
    ...
    # if DS in ['DD']:
        # data_list = init_structural_encoding(data_list, rw_dim=args.hidden_dim, dg_dim=args.hidden_dim)
        # dataset_num_features += args.hidden_dim * 2
